...
contract CampaignFactory is Ownable {
    ...
    function getCampaignAddress(address caller, address superToken, address token, address campaignOperator) public view returns (address) {
        uint256 campaignNonce = campaignNonces[caller];
        bytes32 salt = keccak256(abi.encodePacked(caller, ++campaignNonce));
        bytes32 hash = keccak256(abi.encodePacked(bytes1(0xff), address(this), salt, _getBytecodeHash(superToken, token, campaignOperator)));
